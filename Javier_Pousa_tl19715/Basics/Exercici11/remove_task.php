...
<?php
//Inserto el codi php en aquesta després del botó Actualitzar per a poder
//inserir un misatge a continuació, un cop es faci clic al botó
if($_POST['taskToUpdate'] != ""){
  ...
  $sentencia->bindParam(":id", $taskToUpdate);
  $sentencia->bindParam(":done",$done);
  $sentencia->execute();
  ...
  if ($sentencia->execute()) {
    ...
    echo "Nos s'ha pogut actualitzar la tasca a la BD.";
  }
  //Cerrar conexión con la bd
  $db = null;
}else{
  echo "No has afegit cap nº de tasca.";
}
?>
